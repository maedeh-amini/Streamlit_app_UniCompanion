...
st.markdown("Please evaluate the chatbot's performance using the sliding scales below.\n\n##### **Note: The chatbot generates its response based on the original context.**")
st.markdown("---")
...
with col_context:
    with st.container(border=True):
        st.markdown("##### 📄 Original Context Sources")
        if has_translation:
...
st.write("Please review the Student Question and the three reference panels above, then use the sliders below to answer the following evaluation questions.")

s_faith = st.slider("What percentage of the chatbot's response is directly supported by the original context without making up outside facts?", min_value=0, max_value=100, value=50, step=10, key=f"faith_q_{q_idx}_r_{st.session_state.current_run}")
s_relevancy = st.slider("What percentage of the chatbot's response directly answers the student's question without including unrelated information?", min_value=0, max_value=100, value=50, step=10, key=f"rel_q_{q_idx}_r_{st.session_state.current_run}")
s_recall = st.slider("What percentage of the facts in the expected response does the original context successfully cover?", min_value=0, max_value=100, value=50, step=10, key=f"rec_q_{q_idx}_r_{st.session_state.current_run}")
s_precision = st.slider("What percentage of the original context is directly relevant to answering the student's question?", min_value=0, max_value=100, value=50, step=10, key=f"prec_q_{q_idx}_r_{st.session_state.current_run}")
st.markdown("<br>", unsafe_allow_html=True)
...
if st.button(button_text, use_container_width=True):
    # Pass the slider values directly into the function
